...
function FrameGame1() {
    ...
    const playerStatsRef =useRef({ hp: 10, maxHP: 10, atk: 5, def: 2 });
    ...
    function setMouseSeekMode(value) {
        mouseSeekModeRef.current = value;
    }
    ...
    return (
        ...
        <div className="Frame-Game-1-UI-Row" style={{ display: 'flex', justifyContent: 'space-between', padding: '8px' }}>
            ...
            <ToggleableSwitchComponent
                ...
                onToggle={() => {setMouseSeekMode((current) => !current)
                }}
                label="Mouse Seek: "
            />
            <div className="Frame-Game-1-Stat" style={{marginRight:'0px'}}>
                <div>HP: {playerStatsRef.current.hp} / {playerStatsRef.current.maxHP}</div>
                <div>ATK: {playerStatsRef.current.atk}</div>
                <div>DEF: {playerStatsRef.current.def}</div>
            </div>
