...
import { ENEMY_ARCHETYPES } from '../../configs/enemyArchetypeConfigs.js';
...
export function createCombatSystem({
    ...
    const {
        ...
        onEnemyKilled,
        triggerEffects,
        emitEvent,       // new event path
    } = callbacks;

    function pushDamageFeedback({ targetType, amount, worldX, worldY, color, isPlayer }) {
        if (typeof emitEvent === 'function') {
            emitEvent({
                type: targetType === 'player' ? 'PLAYER_DAMAGED' : 'ENEMY_DAMAGED',
                data: {
                    amount,
                    x: worldX,
                    y: worldY,
    ...
    function playerTakeDamage(amount, x = playerRef.current.x, y = playerRef.current.y, isPlayer = true) {
    ...
    function resolveEnemyHit(enemy, options = {}) {
        ...
        if (enemy.archetype === ENEMY_ARCHETYPES.ARMORED && enemy.shieldHealth > 0) {
            enemy.shieldHealth -= 1; // Always 1 per hit against shield
